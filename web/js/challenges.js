...
async function getRandomChallenge( difficulty = null, excludeIds = [], challengeType = null ) {
    const typeBase = _challengeApiBase( challengeType );
    let url = `${typeBase}/random`;
...
/**
 * Return the API base path for a given challenge type.
 * @param {string|null} challengeType  e.g. 'country', 'city', or null
 */
function _challengeApiBase( challengeType ) {
    if ( challengeType === 'country' ) return `${API_BASE}/challenges/countries`;
    return `${API_BASE}/challenges`;
}
