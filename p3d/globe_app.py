...
class RealGlobeApplication(ShowBase, IGlobeApplication):
    ...
    def zoomIn(self) -> None:
        ...
        self.__guiController.add_log_message(f"ZOOM IN - Distance: {currentDistance:.1f}")

        if currentDistance < 0.1:
            self.__guiController.add_log_message("Camera reset to default position")
            self.camera.setPos(*self.__defaultCameraPos)
            self.camera.lookAt(0, 0, 0)
            return
        ...
        if targetDistance >= currentDistance - 0.1:
            self.__guiController.add_log_message("Already at minimum zoom distance")
            return
        ...
        self.__guiController.add_log_message(f"Zoomed in to distance: {verifyDistance:.1f}")
    ...
    def zoomOut(self) -> None:
        ...
        self.__guiController.add_log_message(f"ZOOM OUT - Distance: {currentDistance:.1f}")
        ...
        if targetDistance <= currentDistance + 0.1:
            self.__guiController.add_log_message("Already at maximum zoom distance")
            return
        ...
        verifyDistance = self.camera.getPos().length()
        self.__guiController.add_log_message(f"Zoomed out to distance: {verifyDistance:.1f}")
    ...
    def resetView(self) -> None:
        ...
        pos = self.camera.getPos()
        distance = pos.length()
        self.__guiController.add_log_message(f"RESET: Distance {distance:.1f} | Rotation X={DEFAULT_ROTATION_X}° Y={DEFAULT_ROTATION_Y}° Z={DEFAULT_ROTATION_Z}°")

    def rotateUp(self) -> None:
        """Rotate globe up by increment amount"""
        self.__globeRotationX += self.__rotationIncrement
        self.__globe.setHpr(self.__globeRotationZ, self.__globeRotationX, self.__globeRotationY)
        self.__guiController.add_log_message(f"UP: X={self.__globeRotationX}° Y={self.__globeRotationY}° Z={self.__globeRotationZ}°")

    def rotateDown(self) -> None:
        """Rotate globe down by increment amount"""
        self.__globeRotationX -= self.__rotationIncrement
        self.__globe.setHpr(self.__globeRotationZ, self.__globeRotationX, self.__globeRotationY)
        self.__guiController.add_log_message(f"DOWN: X={self.__globeRotationX}° Y={self.__globeRotationY}° Z={self.__globeRotationZ}°")

    def rotateLeft(self) -> None:
        """Rotate globe left by increment amount"""
        self.__globeRotationZ -= self.__rotationIncrement
        self.__globe.setHpr(self.__globeRotationZ, self.__globeRotationX, self.__globeRotationY)
        self.__guiController.add_log_message(f"LEFT: X={self.__globeRotationX}° Y={self.__globeRotationY}° Z={self.__globeRotationZ}°")

    def rotateRight(self) -> None:
        """Rotate globe right by increment amount"""
        self.__globeRotationZ += self.__rotationIncrement
        self.__globe.setHpr(self.__globeRotationZ, self.__globeRotationX, self.__globeRotationY)
        self.__guiController.add_log_message(f"RIGHT: X={self.__globeRotationX}° Y={self.__globeRotationY}° Z={self.__globeRotationZ}°")
    ...
    def increaseRotationIncrement(self) -> None:
        ...
        if self.__rotationIncrement < MAX_ROTATION_INCREMENT:
            ...
            self.__guiController.add_log_message(f"Rotation increment increased to {self.__rotationIncrement}°")
        else:
            print(f"Rotation increment: {self.__rotationIncrement}° (MAX)")
            self.__guiController.add_log_message(f"Maximum rotation increment reached ({MAX_ROTATION_INCREMENT}°)")
    ...
    def decreaseRotationIncrement(self) -> None:
        ...
        if self.__rotationIncrement > MIN_ROTATION_INCREMENT:
            self.__rotationIncrement -= 1
            print(f"Rotation increment: {self.__rotationIncrement}°")
            self.__guiController.add_log_message(f"Rotation increment decreased to {self.__rotationIncrement}°")
        else:
            print(f"Rotation increment: {self.__rotationIncrement}° (MIN)")
            self.__guiController.add_log_message(f"Minimum rotation increment reached ({MIN_ROTATION_INCREMENT}°)")
    ...
    def setPresetView(self, index: int) -> None:
        ...
        if 0 <= index < len(self.__presetViews):
            ...
            self.__globe.setHpr(self.__globeRotationZ, self.__globeRotationX, self.__globeRotationY)
            self.__guiController.add_log_message(f"{chosenView['name']}: X={self.__globeRotationX}° Y={self.__globeRotationY}° Z={self.__globeRotationZ}°")
